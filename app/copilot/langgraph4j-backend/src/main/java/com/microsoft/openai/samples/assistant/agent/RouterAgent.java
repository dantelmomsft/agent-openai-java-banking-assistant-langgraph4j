...
public class RouterAgent {
    ...
    private final ToolsExecutionCache<Object> toolsExecutionCache;

    public RouterAgent(LoggedUserService loggedUserService, ToolsExecutionCache<Object> toolsExecutionCache, OpenAIAsyncClient openAIAsyncClient, DocumentIntelligenceClient documentIntelligenceClient, BlobStorageProxy blobStorageProxy, @Value("${openai.chatgpt.deployment}") String gptChatDeploymentModelId, @Value("${transactions.api.url}") String transactionsAPIUrl, @Value("${accounts.api.url}") String accountsAPIUrl, @Value("${payments.api.url}") String paymentsAPIUrl ){
    ...
    private void routeToAgent(IntentResponse intentResponse, ChatHistory chatHistory, AgentContext agentContext) {
        if(agentContext == null)
            agentContext = new AgentContext( Map.of() );
        ...
        switch (intentResponse.getIntentType()) {
            ...
            case RepeatTransaction:
                ...
                agentContext.result().ifPresent(chatHistory::addAssistantMessage);
                break;
